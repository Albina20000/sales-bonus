...
function analyzeSalesData(data, options) {
    ...
    for (const record of data.purchase_records) {
        ...
        for (const item of record.items) {
            ...
            receiptRevenue += calculateRevenue(item, product);
            receiptCost += product.purchase_price * item.quantity;
        ...
        // Округляем итоговые суммы по чеку
        seller.revenue += Math.round(receiptRevenue * 100) / 100;
        seller.total_cost += Math.round(receiptCost * 100) / 100;
    }
