...
function analyzeSalesData(data, options) {
    ...
    for (const record of data.purchase_records) {
        ...
        // Выручка по чеку: общая сумма минус общая скидка
        const revenueFromRecord = record.total_amount - record.total_discount;
        seller.revenue = Math.round((seller.revenue + revenueFromRecord) * 100) / 100;
