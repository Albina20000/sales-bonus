function calculateSimpleRevenue(purchase, _product) {
    // Округляем КАЖДОЕ умножение
    const salePrice = Math.round(purchase.sale_price * 100) / 100;
    const quantity = purchase.quantity;
    const discount = 1 - (purchase.discount / 100);
    return Math.round(salePrice * quantity * discount * 100) / 100;
}
...
function analyzeSalesData(data, options) {
    ...
    // Обработка чеков (округляем КАЖДУЮ операцию)
    ...
    for (let i = 0; i < sellerStats.length; i++) {
        ...
        const bonus = Math.round(calculateBonus(i, total, seller) * 100) / 100;
